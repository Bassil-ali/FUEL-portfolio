...
@section('content')
    ...
    <ul class="breadcrumb mt-2">
        ...
        <li class="breadcrumb-item"><a class="back-page" href="{{ route('admin.success_partners.index') }}">@lang('site.success_partners')</a></li>
        <li class="breadcrumb-item">@lang('site.create')</li>
    </ul>


    <form method="post" action="{{ route('admin.success_partners.store') }}" enctype="multipart/form-data">
        @csrf
        @method('post')
        ...
        <div class="row">
            ...
            <div class="col-12 col-md-4">
                ...
                <div class="tile shadow">
                    ...
                    @include('admin.dataTables.image_privew')
                        ...
                        @foreach(getLanguages() as $language)
                          <div class="tab-pane fade {{ $loop->first ? 'show active' : '' }}" id="{{ $language->code }}" role="tabpanel" aria-labelledby="{{ $language->code }}-tab">
                            ...
                            <div class="form-group">
                                ...
                                <textarea id="description-{{ $language->code }}" class="form-control @error('description.' . $language->code) is-invalid @enderror" name="description[{{ $language->code }}]" rows="5">{{ old('description.' . $language->code) }}</textarea>
                   ...
                   {{--status--}}
                    ...
                    <div class="form-group ml-3">
                        <div class="form-check form-switch">
                          <input class="form-check-input" id="status" type="checkbox" name="status" value="true" {{ old('status') ? 'checked' : '' }}>
                        </div>
                    </div>
