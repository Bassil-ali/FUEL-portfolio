...
@section('content')
    ...
    <ul class="breadcrumb mt-2">
        ...
        <li class="breadcrumb-item">@lang('site.sucess_partners')</li>
    </ul>
                ...
                <div class="row mb-2">
                    ...
                    <div class="col-md-12">
                    	...
                    	@if(permissionAdmin('create-partners'))
                            <a href="{{ route('admin.success_partners.create') }}" class="btn btn-primary">
                            	<i class="fa fa-plus"></i> @lang('site.create')
                            </a>
						@endif
                        @if(permissionAdmin('delete-partners'))
                            <form method="post" action="{{ route('admin.success_partners.bulk_delete') }}" style="display: inline-block;">
                                @csrf
                                @method('post')
...
@push('scripts')
    ...
    <script>
        ...
        let dataTable = $('#data-table').DataTable({
            ...
            ajax: {
                url: '{{ route('admin.success_partners.data') }}',
            },
            columns: [
        ...
        $(document).on('change', '.status', function (e) {
            ...
            let url    = "{{ route('admin.success_partners.status') }}";
            let method = 'post';
            let id     = $(this).data('id');
