                        ...
                        @foreach(getLanguages() as $language)
                          <div class="tab-pane fade {{ $loop->first ? 'show active' : '' }}" id="{{ $language->code }}" role="tabpanel" aria-labelledby="{{ $language->code }}-tab">
                            ...
                            <div class="form-group">
                                ...
                                <textarea id="description-{{ $language->code }}" class="form-control @error('description.' . $language->code . '.' . $loop->index) is-invalid @enderror" name="description[{{ $language->code }}]" rows="5">{{ old('description.' . $language->code) }}</textarea>
